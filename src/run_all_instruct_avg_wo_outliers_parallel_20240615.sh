        ...
        for replace in "${replace_array[@]}"; do
            ...
            for ((i = 0; i < total_prompts; i += concurrent_jobs)); do
                for ((j = i; j < i + concurrent_jobs; j++)); do
                    ...
                    echo "Extracted filename: $modelname"
                    python cal_bias_auto_avg.py \
                                        -m $model \
                                        -p $prompt \
                                        -tp $TP \
                                        -o $origin \
                                        -bp "/home/u3659277/master-thesis/TWBias/data/ethical_group/result_wo_outliers/${modelname}/${origin_dict[$origin]}_$replace/${modelname}_${origin_dict[$origin]}_${replace}_output_$prompt.csv" \
